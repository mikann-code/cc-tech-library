...
date: 2026-05-29
tags: [rails, ruby, job, active-job, solid-queue, design, recurring, cron]
---
...
## 追記：recurring.yml で定期実行を宣言的に設定する

> 上記は Job 全体像（§1–13）と「なぜ Job に切り出すか（8 メリット）」のまとめ。ここでは `recurring.yml`（Solid Queue の定期実行設定）そのものにフォーカスして深掘りする。題材は `EventReminderJob`（リマインダーメール）。

### 概要

`recurring.yml` は Solid Queue の「定期実行ジョブ（cron 相当）」を宣言的に設定するファイル。「いつ・どのジョブを・どんな引数で動かすか」を YAML で書く。

- 場所: `config/recurring.yml`

### ファイル構造

```yaml
development:        # 環境ごとにブロックが分かれる
  タスク名:          # 任意のラベル（識別用）
    class: ジョブクラス
    args: [引数]
    schedule: 実行タイミング
...
- 一番外側は環境名（development / production）。Rails は現在の環境に一致するブロックだけ読む
- その下は「タスク名 → 定義」の繰り返し。タスク名は自由に付けられる識別子

### 使えるキー

| キー | 意味 |
|---|---|
| `class` | 実行する Job クラス（`perform` が呼ばれる） |
| `command` | クラスの代わりに Ruby コードを直接実行（ワンライナー向け） |
| `args` | `perform` に渡す引数（配列） |
| `schedule` | 実行タイミング（必須） |
| `queue` | 投入先キュー（任意） |
| `priority` | 優先度（任意） |

- `class` と `command` はどちらか一方

### args は perform の引数になる

```yaml
event_reminder_3days:
  class: EventReminderJob
  args: [3]          # → perform(3)、days_before = 3
```

```ruby
def perform(days_before)   # 3 が入る
  target_date = Date.current + days_before.days
end
```

- 同じジョブを引数違いで複数登録することで、「3 日前」「1 日前」を 1 つのクラスで実現できる
- 将来「7 日前」を足したいときも YAML に 1 ブロック追加するだけ

### schedule の書き方

Fugit が解釈。自然言語と cron 形式の両方 OK。

| 例 | 意味 |
|---|---|
| `every hour` | 毎時 0 分 |
| `every hour at minute 12` | 毎時 12 分 |
| `at 4am every day` | 毎日 4 時 |
| `every day at 0:00am` | 毎日 0 時 |
| `"0 4 * * *"` | （cron）毎日 4 時 |

### ⚠️ タイムゾーンの罠（最重要）

- アプリ本体は JST（`config.time_zone = "Tokyo"`）
- しかし `schedule` の時刻は UTC で解釈される（このプロジェクトのコメント前提）
- JST = UTC + 9 時間

> YAML に書く時刻（UTC）＝ 送りたい時刻（JST）− 9 時間

| リマインダー | YAML（UTC） | 実際の送信（JST） |
|---|---|---|
| 3 日前 | `0:00am` | 9:00（朝） |
| 1 日前 | `10:00am` | 19:00（夜） |

- 0 時と 9 時は別の実行ではなく、同じ瞬間を UTC と JST で二重表記しているだけ
- コメント `# JST 09:00 = UTC 00:00` は換算ミス防止のメモ
- ⚠️ 時刻変更時は必ず「JST 希望時刻 − 9h → UTC で記入」。忘れると 9 時間ズレる
- （注）UTC 解釈はバージョン・設定で変わりうるので、確実を期すなら実環境で起動時刻を検証

### 3 日前 / 1 日前を分ける理由（※推測・ドキュメント未記載）

| 観点 | 3 日前（朝 9 時） | 1 日前（夜 19 時） |
|---|---|---|
| 目的 | 準備・予定確認を促す | 直前の最終確認 |
| 時刻の意図 | 1 日の始まりに準備行動を促せる | 夜は翌日の予定を確認する時間帯 |

- 原則：「メールで取ってほしい行動の時間帯」に送信時刻を合わせる
- ⚠️ この時刻差の正式な根拠はコード・ドキュメントに残っていない（要確認 → 確定したらコメントに残すと良い）

### 実行インフラまわり

| 役割 | 担当 | AWS か |
|---|---|---|
| 定期実行のスケジューリング | Solid Queue（MySQL） | ❌ AWS 非依存 |
| ジョブ実行 | worker プロセス（`app_worker`） | AWS サーバー上で動く |
| メール実配信 | AWS SES v2（東京リージョン） | ✅ AWS |

- Solid Queue は無料（MIT・Rails 8 標準同梱）。Redis 不要 = 追加インフラ 0 円
- ただし worker が常時起動していることが前提（落ちるとスケジュールも止まる → systemd 等で常駐管理）
- メール配信のみ AWS SES（`delivery_method = :ses_v2`、開発環境でも実送信。例外は `E2E=true` 時のみ `:test`）

### Solid Queue と Rails の関係

```
Active Job（Rails 本体）= ジョブの共通インターフェース
   └─ アダプタを差し替え可能
        ├─ :async   … Rails 標準・プロセス内・揮発性（本番不可）
        ├─ Sidekiq  … Redis 必須
        └─ Solid Queue … MySQL で動く（Rails 8 デフォルト）★
```

- `deliver_later` 等は Active Job（Rails 標準）の機能
- 定期実行（recurring）は Solid Queue 固有の機能（Active Job 単体にはない）

### 全体の処理フロー

```
recurring.yml（Solid Queue・MySQL）
  └─ EventReminderJob#perform(days_before)
       └─ EventApplicationMailer.reminder(...).deliver_later  ← Active Job
            └─ worker が処理
...
- `recurring.yml` は Solid Queue 固有の宣言的な定期実行設定（cron 相当）。環境ブロック → タスク名 → `class`/`args`/`schedule` の構造
- `args` は `perform` の引数になる → 同じジョブを引数違いで複数登録でき、「3 日前」「1 日前」を 1 クラスで実現できる
- `schedule` は Fugit が解釈し、自然言語（`at 4am every day`）と cron（`"0 4 * * *"`）の両対応
- ⚠️ 最重要の罠：`schedule` の時刻は UTC 解釈。JST で送りたい時刻から 9 時間引いて書く。間違えると 9 時間ズレる
- 「3 日前は朝 / 1 日前は夜」の時刻差は推測であり、正式な根拠はコード・ドキュメントに残っていない（要確認）
- インフラはスケジューリング = Solid Queue（MySQL・AWS 非依存）、メール実配信のみ AWS SES v2（東京）。worker が常時起動していることが前提
